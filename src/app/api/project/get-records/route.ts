...
export async function GET(req: Request) {
    try {
        ...
        const project = projectData.data?.Voice_Studio_projects[0];
        if (!project) {
            // This can happen if the project doesn't exist OR if the user doesn't have permission
            return NextResponse.json({ error: "Project not found or access denied" }, { status: 404 });
        }

        let allBlocks = project.blocks_json || [];
        if (allBlocks && !Array.isArray(allBlocks) && allBlocks.blocks) {
            allBlocks = allBlocks.blocks;
        } else if (allBlocks && !Array.isArray(allBlocks)) {
            allBlocks = [];
        }
